...
let numRandom = Math.floor(Math.random() * 21);
...
console.log(splitMe("Ho fatto schifo al quiz"));
/* ESERCIZIO 4
...
console.log(onlyLetters("Wrath995"));
...
console.log(isThisAnEmail("pippo@baudo.it"));
/* ESERCIZIO 7
...
const whatDayIsIt = () => {
  ...
  let oggi = new Date();
  let dayOfWeekNum = oggi.getDay();
  return weekDays[dayOfWeekNum];
};
...
const howManyDays = (stringaData) => {
  let oggi = new Date();
  let inserimentoData = new Date(stringaData);
  let diffInMillisecondi = oggi - inserimentoData;
  let diffInDays = Math.floor(diffInMillisecondi / (1000 * 60 * 60 * 24));
  return diffInDays;
};

console.log(howManyDays("2024-04-28"));
/* ESERCIZIO 10
...
const isTodayMyBirthday = () => {
  ...
  let giornoCorrente = oggi.getDate();
  if (meseCorrente === myBirth.mese && giornoCorrente === myBirth.giorno) {
    return true;
  } else {
    return false;
  }
...
const deleteProp = (oggetto, proprietà) => {
...
console.log(deleteProp(student, "età"));
/* ESERCIZIO 12
...
const newestMovie = (movies) => {
  let recentFilm = movies[0];
  for (let i = 1; i < movies.length; i++) {
    if (parseInt(movies[i].Year) > parseInt(recentFilm.Year)) {
      recentFilm = movies[i];
    }
  }
  return recentFilm;
...
const searchByTitle = (titleString, movies) => {
  return movies.filter((movie) => movie.Title.includes(titleString));
};
console.log(searchByTitle("Avenger", movies));
/* ESERCIZIO 18
...
const removeIndex = (index, movies) => {
  return movies.filter((movie, idx) => idx !== index);
};
let newMovies = removeIndex(2, movies);
